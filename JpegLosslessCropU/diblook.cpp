...
void load_inf(void)
{
	...
	if ((fp = _tfopen(theApp.m_infname, _T("r"))) == NULL)
		return;
...
void save_inf(void)
{
	...
	if ((fp = _tfopen(theApp.m_infname, _T("w"))) == NULL)
		return;
...
BOOL CDibLookApp::InitInstance()
{
	...
	if (lstrlen(m_infname) && m_infname[lstrlen(m_infname) - 1] != '\\')
		lstrcat(m_infname, _T("\\"));
	lstrcat(m_infname, _T("jpegcrop.inf"));
	load_inf();
