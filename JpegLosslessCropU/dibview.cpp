...
void CDibView::OnViewZoom_helper(int selectedLevel)
{
	...
	{
		if (pDoc->m_scale == selectedLevel) return;
		BeginWaitCursor();
		pDoc->ReplaceHDIB(NULL);

		HDIB hDIB = (HDIB)Read_JPEG_File(pDoc->GetPathName(),
			selectedLevel, pDoc->m_force_grayscale,
	...
	else if (pDoc->m_scale != pDoc->m_scale_denom)
	{
		...
		pDoc->InitDIBData();
		EndWaitCursor();
		pDoc->UpdateAllViews(NULL, selectedLevel + 3);
	}
	else
	{
		if (m_zoomval == selectedLevel) return;
		OnUpdate(NULL, selectedLevel + 3);
	}
}
...
void CDibView::OnDraw(CDC* pDC)
{
	...
	if (m_format[3] == 'r')
		pDoc->m_crop_width_set = 4;
	else if (m_format[3] == 'f')
		pDoc->m_crop_width_set = 3;
	else
		pDoc->m_crop_width_set = 1;
}
...
// Dialog Data
	...
	int m_xoffset;
	int m_yoffset;
	const TCHAR*m_format1;
	const TCHAR*m_format2;
	const TCHAR*m_format3;
	const TCHAR*m_format4;
// Implementation
protected:
...
BOOL CFrameDlg::OnInitDialog()
{
	...
	_stprintf(buffer, m_format1, width);
	SetDlgItemText(IDC_EDIT1, buffer);
	_stprintf(buffer, _T("%d"), height);
	SetDlgItemText(IDC_EDIT2, buffer);
	if (xoffset < 0)
		buffer[0] = 0;
	else
		_stprintf(buffer, _T("%d"), xoffset);
	SetDlgItemText(IDC_EDIT3, buffer);
	if (yoffset < 0)
		buffer[0] = 0;
	else
		_stprintf(buffer, _T("%d"), yoffset);
	SetDlgItemText(IDC_EDIT4, buffer);
	if (xoffset < 0)
		_stprintf(buffer, m_format2, width, height);
	else if (yoffset < 0)
		_stprintf(buffer, m_format3, width, height, xoffset);
	else
		_stprintf(buffer, m_format4 + 1, width, height, xoffset, yoffset);
...
void CFrameDlg::OnOK()
{
	...
	{
		char buffer_a[128]; //not changing jtransform_parse_crop_spec to use TCHAR
		::GetDlgItemTextA(m_hWnd,IDC_EDIT5, buffer_a, sizeof(buffer_a));
		if (jtransform_parse_crop_spec(&cropinfo, buffer_a))
		{
			switch (cropinfo.crop_width_set)
			{
			case JCROP_POS:
				_stprintf(buffer, _T("%d"), cropinfo.crop_width);
				SetDlgItemText(IDC_EDIT1, buffer);
				break;
			case JCROP_FORCE:
				_stprintf(buffer, _T("%df"), cropinfo.crop_width);
				SetDlgItemText(IDC_EDIT1, buffer);
				break;
			case JCROP_REFLECT:
				_stprintf(buffer, _T("%dr"), cropinfo.crop_width);
				SetDlgItemText(IDC_EDIT1, buffer);
				break;
			default:
				break;
			...
			{
				_stprintf(buffer, _T("%d"), cropinfo.crop_height);
				SetDlgItemText(IDC_EDIT2, buffer);
			}
			if (cropinfo.crop_xoffset_set == JCROP_POS)
			{
				_stprintf(buffer, _T("%d"), cropinfo.crop_xoffset);
				SetDlgItemText(IDC_EDIT3, buffer);
			}
			if (cropinfo.crop_yoffset_set == JCROP_POS)
			{
				_stprintf(buffer, _T("%d"), cropinfo.crop_yoffset);
				SetDlgItemText(IDC_EDIT4, buffer);
			}
		}
	...
	if (len)
	{
		if (buffer[len - 1] == 'r' || buffer[len - 1] == 'R')
		{
			buffer[len - 1] = 0;
			m_format4 = _T(" %drx%d+%d+%d");
		}
		else if (buffer[len - 1] == 'f' || buffer[len - 1] == 'F')
		{
			buffer[len - 1] = 0;
			m_format4 = _T(" %dfx%d+%d+%d");
		}
		else
			m_format4 = _T(" %dx%d+%d+%d");
	}
	((CDibLookApp *)AfxGetApp())->m_width = _ttoi(buffer);
	GetDlgItemText(IDC_EDIT2, buffer, sizeof(buffer));
	((CDibLookApp *)AfxGetApp())->m_height = _ttoi(buffer);
	GetDlgItemText(IDC_EDIT3, buffer, sizeof(buffer));
	if (buffer[0])
		((CDibLookApp *)AfxGetApp())->m_xoffset = _ttoi(buffer);
	GetDlgItemText(IDC_EDIT4, buffer, sizeof(buffer));
	if (buffer[0])
		((CDibLookApp *)AfxGetApp())->m_yoffset = _ttoi(buffer);
}
...
void CFrameDlg::OnCheck1()
{
	...
	if (((CButton *)GetDlgItem(IDC_CHECK1))->GetCheck())
	{
		...
		if (buffer3[0] == 0)
			_stprintf(buf, _T("%sx%s"), buffer1, buffer2);
		else if (buffer4[0] == 0)
			_stprintf(buf, _T("%sx%s+%s"), buffer1, buffer2, buffer3);
		else
			_stprintf(buf, _T("%sx%s+%s+%s"), buffer1, buffer2, buffer3, buffer4);
	...
	{
		char buf_a[64]; //not changing jtransform_parse_crop_spec to use TCHAR
		::GetDlgItemTextA(m_hWnd,IDC_EDIT5, buf_a, sizeof(buf_a));
		if (jtransform_parse_crop_spec(&cropinfo, buf_a))
		{
			switch (cropinfo.crop_width_set)
			{
			case JCROP_POS:
				_stprintf(buffer1, _T("%d"), cropinfo.crop_width);
				SetDlgItemText(IDC_EDIT1, buffer1);
				break;
			case JCROP_FORCE:
				_stprintf(buffer1, _T("%df"), cropinfo.crop_width);
				SetDlgItemText(IDC_EDIT1, buffer1);
				break;
			case JCROP_REFLECT:
				_stprintf(buffer1, _T("%dr"), cropinfo.crop_width);
				SetDlgItemText(IDC_EDIT1, buffer1);
				break;
			default:
				break;
			...
			if (cropinfo.crop_height_set == JCROP_POS)
			{
				_stprintf(buffer2, _T("%d"), cropinfo.crop_height);
				SetDlgItemText(IDC_EDIT2, buffer2);
			}
			if (cropinfo.crop_xoffset_set == JCROP_POS)
			{
				_stprintf(buffer3, _T("%d"), cropinfo.crop_xoffset);
				SetDlgItemText(IDC_EDIT3, buffer3);
			}
			if (cropinfo.crop_yoffset_set == JCROP_POS)
			{
				_stprintf(buffer4, _T("%d"), cropinfo.crop_yoffset);
...
void CDibView::OnPopup1DefineFrame()
{
	...
	if (m_format[3] == 'r')
	{
		frameDlg.m_format1 = _T("%dr");
		frameDlg.m_format2 = _T("%drx%d");
		frameDlg.m_format3 = _T("%drx%d+%d");
	}
	else if (m_format[3] == 'f')
	{
		frameDlg.m_format1 = _T("%df");
		frameDlg.m_format2 = _T("%dfx%d");
		frameDlg.m_format3 = _T("%dfx%d+%d");
	}
	else
	{
		frameDlg.m_format1 = _T("%d");
		frameDlg.m_format2 = _T("%dx%d");
		frameDlg.m_format3 = _T("%dx%d+%d");
	}
	frameDlg.m_format4 = m_format;

	if (frameDlg.DoModal() != IDOK) return;

	m_format = frameDlg.m_format4;
...
void CDibView::OnViewZoom1() { OnViewZoom_helper(1); }
void CDibView::OnViewZoom2() { OnViewZoom_helper(2); }
void CDibView::OnViewZoom3() { OnViewZoom_helper(3); }
void CDibView::OnViewZoom4() { OnViewZoom_helper(4); }
void CDibView::OnViewZoom5() { OnViewZoom_helper(5); }
void CDibView::OnViewZoom6() { OnViewZoom_helper(6); }
void CDibView::OnViewZoom7() { OnViewZoom_helper(7); }
void CDibView::OnViewZoom8() { OnViewZoom_helper(8); }
void CDibView::OnViewZoom9() { OnViewZoom_helper(9); }
void CDibView::OnViewZoom10() { OnViewZoom_helper(10); }
void CDibView::OnViewZoom11() { OnViewZoom_helper(11); }
void CDibView::OnViewZoom12() { OnViewZoom_helper(12); }
void CDibView::OnViewZoom13() { OnViewZoom_helper(13); }
void CDibView::OnViewZoom14() { OnViewZoom_helper(14); }
void CDibView::OnViewZoom15() { OnViewZoom_helper(15); }
void CDibView::OnViewZoom16() { OnViewZoom_helper(16); }
...
void CDibView::OnUpdateToolbarScale(CCmdUI *pCmdUI)
{
	...
	_stprintf(buf, _T("%d/%d"), scale, pDoc->m_scale_denom);
	pCmdUI->SetText(buf);
}
